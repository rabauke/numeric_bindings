...
int main(int argc, char *argv[]) {
  {
    ...
    {
      matrix C1(alpha*ublas::prod(A, ublas::trans(ublas::conj(A)))+beta*C);
      for (size_type j=0; j<n; ++j)
	for (size_type i=j+1; i<n; ++i)
	  C1(i, j)=C(i, j);
      matrix C2(C);
      blas::herk(alpha, A, beta, blas::upper(C2));
    ...
    }
    {
      ...
      for (size_type j=0; j<n; ++j)
    	for (size_type i=0; i<j; ++i)
    	  C1(i, j)=C(i, j);
      matrix C2(C);
      blas::herk(alpha, A, beta, blas::lower(C2));
      std::cout << "testing boost::ublas containers\n"
    ...
    {
      matrix C1(alpha*ublas::prod(ublas::trans(ublas::conj(A)), A)+beta*C);
      for (size_type j=0; j<n; ++j)
    	for (size_type i=j+1; i<n; ++i)
    	  C1(i, j)=C(i, j);
      matrix C2(C);
      blas::herk(alpha, blas::conj(A), beta, blas::upper(C2));
      std::cout << "testing boost::ublas containers\n"
    ...
    }
    {
      ...
      for (size_type j=0; j<n; ++j)
    	for (size_type i=0; i<j; ++i)
    	  C1(i, j)=C(i, j);
      matrix C2(C);
      blas::herk(alpha, blas::conj(A), beta, blas::lower(C2));
      std::cout << "testing boost::ublas containers\n"
